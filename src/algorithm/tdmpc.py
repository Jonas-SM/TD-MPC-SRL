...
class TOLD(nn.Module):
	...
	def h_inverse(self, obs):
		"""Decodes a latent representation and returns an estimate of the original observation"""
...
class TDMPC():
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		...
		for i in range(self.cfg.iterations):
			...
			# we select top-k returns and obtain new parameters' mu (mean) and sigma (standard deviation)
		...
		# Return reconstructed observation as additional output
		z = self.model.h(obs)
		est_obs = self.model.h_inverse(z)
	...
	def update(self, replay_buffer, step):
		...
		consistency_loss, reward_loss, value_loss, priority_loss, reconstruction_loss = 0, 0, 0, 0, 0
		for t in range(self.cfg.horizon):
			...
			with torch.no_grad():
				next_obs_noaug = next_obses[t]
				next_obs = self.aug(next_obses[t])

				next_z_noaug = self.model_target.h(next_obs_noaug)
				next_z = self.model_target.h(next_obs)
				td_target = self._td_target(next_obs, reward[t])

			zs.append(z.detach())

			# reconstructed observation (h-1(z))
			est_obs = self.model.h_inverse(next_z_noaug)
			...
			reconstruction_loss += rho * torch.square(est_obs - next_obs_noaug).flatten(start_dim=1).mean(1)

		# Optimize model
		total_loss = self.cfg.consistency_coef * consistency_loss.clamp(max=1e4) + \
					 self.cfg.reconstruction_coef * reconstruction_loss.clamp(max=1e4) + \
