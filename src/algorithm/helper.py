...
def buildMLP(input_size, layer_sizes, output_size, output_activation=torch.nn.Identity, activation=torch.nn.ELU):
...
class reconstruct(nn.Module):
	def __init__(self, cfg):
		super().__init__()
		self.C = int(3 * cfg.frame_stack)
		self.cfg = cfg

		self.l1 = nn.Linear(cfg.latent_dim, (self.C*5*5))

		self.deconv1 = nn.ConvTranspose2d(self.C, self.cfg.num_channels, 2, stride=2)
		self.deconv2 = nn.ConvTranspose2d(self.cfg.num_channels, self.cfg.num_channels, 2, stride=2)
		self.deconv3 = nn.ConvTranspose2d(self.cfg.num_channels, self.cfg.num_channels, 4, stride=2)
		self.deconv4 = nn.ConvTranspose2d(self.cfg.num_channels, 9, 2, stride=2)

		self.bn1 = nn.BatchNorm2d(self.cfg.num_channels)
		self.bn2 = nn.BatchNorm2d(self.cfg.num_channels)
		self.bn3 = nn.BatchNorm2d(self.cfg.num_channels)

	def forward(self, x):
		x = F.relu(self.l1(x)).view(x.size()[0], self.C, 5, 5)
...
def dec(cfg):
	...
	Model reconstructs the original observation from the given latent space
	"""
	if cfg.modality == 'pixels':
		mlp = reconstruct(cfg)
	...
	layers = [
		...
		nn.ELU(),
		nn.Linear(cfg.enc_dim, cfg.obs_shape[0]),
	]
